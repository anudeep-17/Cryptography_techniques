...
protected static byte[][] w;
 ...
 protected static int[] sbox = { 0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F,
 ...
 protected static int[] inv_sbox = { 0x52, 0x09, 0x6A, 0xD5, 0x30, 0x36, 0xA5,
 ...
 protected static int Rcon[] = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a, 
 ...
 protected static byte[] xor_func(byte[] a, byte[] b) {
 ...
 protected static byte[][] generateSubkeys(byte[] key) {
 ...
 protected static byte[] SubWord(byte[] in) {
 ...
 protected static byte[] rotateWord(byte[] input) {
 ...
 protected static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {
 ...
 protected static byte[][] SubBytes(byte[][] state) {
 ...
 protected static byte[][] InvSubBytes(byte[][] state) {
  for (int row = 0; row < 4; row++) 
   for (int col = 0; col < Nb; col++)
 ...
 protected static byte[][] ShiftRows(byte[][] state) {
 ...
 protected static byte[][] InvShiftRows(byte[][] state) { 
  byte[] t = new byte[4]; 
  for (int r = 1; r < 4; r++) {
   for (int c = 0; c < Nb; c++) 
    t[(c + r)%Nb] = state[r][c];
 ...
 protected static byte[][] InvMixColumns(byte[][] s){
   int[] sp = new int[4];
 ...
 protected static byte[][] MixColumns(byte[][] s){
   int[] sp = new int[4];
...
static byte[] bitgetter(byte[] temp, int index)
 ...
 public static byte[] encrypt(byte[] in,byte[] key, char... optionaltype){
